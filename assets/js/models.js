...
const asciMinusnum = 65; // asci value variable
const yMinusnum = 1; // converting for y coordinate
...
var battleship = {
  ...
  canIPlaceShip: function (coordinates, orientation, size) {
    if (orientation === "x") {
      ...
      if (coordinates[0] + size >= boardXsize) return false;
    } else {
      //if board falls outside x coordinates
      if (coordinates[1] + size >= boardYsize) return false;
    }
    //if there is an overlap, return false
    for (let i = 0; i < size; i++) {
      if (orientation === "x") {
        if (this.Board[coordinates[0] + i][coordinates[1]] !== "") return false;
      } else {
        if (this.Board[coordinates[0]][coordinates[1] + i] !== "") return false;
      }
    }
    return true;
  },
      ...
      for (let i = 0; i < allVessels.length; i++) {
        if (allVessels[i].getName() === vesselName) {
          ...
          if (allVessels[i].getHits() - 1) {
            ...
            //update message div
            addMessage("and you hit a " + vesselName + "!");
            addMessage("");
          } else {
            addMessage("and you sunk a " + vesselName + "!");
            addMessage("");
          }
        }
      }
  ...
  //gets called by the respective listener and calls makeMove() by passing appropriate parameters.
  getRemoteMove: function (x, y) {
    // if x or y in undefined do nothing
    if (x === undefined || y == undefined) {
    }
    //else do follow the following steps
    else {
      addMessage(`You play [${x}, ${y}] coordinates`);
      let xco = x.charCodeAt(0) - asciMinusnum;
      let yco = y - 1;
      var arrayCoordinate = [xco, yco];
      console.log("this is array: " + arrayCoordinate);
      return battleship.makeMove(arrayCoordinate);
    }
  },
};
